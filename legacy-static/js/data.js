/* ===========================================================
   MAF · Datos simulados
   Todo lo que se ve en la maqueta sale de este archivo.
   Los contactos se generan con un PRNG con semilla fija, para
   que los conteos de los segmentos sean reales y reproducibles.
   =========================================================== */
...
(function () {
  ...
  function sumarDias(d, n) { return restarDias(d, -n); }
  ...
  function evaluarSegmento(reglas, logica) {
    if (!reglas || !reglas.length) return [];
    return contactos.filter(function (c) {
      if (logica === 'or') return reglas.some(function (r) { return cumpleRegla(c, r); });
      return reglas.every(function (r) { return cumpleRegla(c, r); });
    });
  }
  ...
  segmentos.forEach(function (s) {
    ...
    s.tamano = res.length;
    s.conOptIn = res.filter(function (c) { return c.consentimiento === 'vigente'; }).length;
  });

  /* ---------- Precios Meta (USD por conversación, Chile) ---------- */
  var precios = { marketing: 0.0592, utility: 0.0119, authentication: 0.0357, service: 0 };
  ...
  /* ---------- Conversaciones ---------- */
  function h(hora, min) { var d = new Date(HOY.getTime()); d.setHours(hora, min, 0, 0); return d; }
  function hAyer(hora, min) { var d = restarDias(HOY, 1); d.setHours(hora, min, 0, 0); return d; }
  function hDias(dias, hora, min) { var d = restarDias(HOY, dias); d.setHours(hora, min, 0, 0); return d; }
  ...
  function porCampana(id) { return campanas.filter(function (c) { return c.id === id; })[0]; }

  function filaComercial(nombre, ids, tasaCierre) {
    var cs = ids.map(porCampana);
    var f = { nombre: nombre, contactados: 0, respondieron: 0, calificados: 0, cerrados: 0, costoUsd: 0 };
